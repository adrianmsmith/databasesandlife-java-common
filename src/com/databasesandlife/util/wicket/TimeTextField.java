 ...
 * 
 * <p>{@link java.sql.Time} is adjusted with the timezone to make it compatible with PostgreSQL and jOOQ.
 * 
    ...
    public <C> IConverter<C> getConverter(Class<C> type) {
        if (type.equals(Time.class)) return (IConverter<C>) new IConverter<Time>() {
            @Override public Time convertToObject(String str, Locale l) {
                return new Time(LocalTime.parse(str, DateTimeFormatter.ofPattern("HH:mm")).get(ChronoField.MILLI_OF_DAY)
                    - TimeZone.getDefault().getOffset(new Date().getTime()));
            }

            @Override public String convertToString(Time time, Locale l) {
                long millisOfDay = time.getTime() + TimeZone.getDefault().getOffset(new Date().getTime());
                return time == null ? "" : LocalTime.ofSecondOfDay(millisOfDay/1000).format(DateTimeFormatter.ofPattern("HH:mm"));
            }
        };
